...
int rbTransmit(uint8_t *rbPtr, uint16_t rbLen, uint8_t rbType) {d
...
//#endif // SERIAL_DEBUG_ROCKBLOCK
  ...
  if (rbType == RBQ_MSG_TYPE_BIN || rbType == RBQ_MSG_TYPE_SENSOR) {
    rc = isbd.sendSBDBinary((rbPtr, rbLen);
  } else {
    rc = isbd.sendSBDchar(rbPtr);
  }
//    }
//    }
...
int rbTransmitDataNow(uint8_t *rbPtr, int rbLen, int rbType) {
  ...
  if((rc = rbInit()) == ISBD_SUCCESS) {
    rc = rbTransmitData(rbPtr, rbLen, rbType);
  }
